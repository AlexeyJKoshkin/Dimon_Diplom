...
public static class SpPacker
{
    ...
    {
        width = Mathf.Clamp(width, 2, SpAtlas.MAX_ATLAS_SIZE);
        height = Mathf.Clamp(height, 2, SpAtlas.MAX_ATLAS_SIZE);
    ...
    private static void AutoPackLarger(bool forceSquare, ref List<SpRect> testRects, ref List<SpRect> rects, ref int width, ref int height)
    {
        ...
        while (true)
        {
            ...
            // Too big?
            if (testWidth > SpAtlas.MAX_ATLAS_SIZE || testHeight > SpAtlas.MAX_ATLAS_SIZE)
            {
                width = -1;
                height = -1;
