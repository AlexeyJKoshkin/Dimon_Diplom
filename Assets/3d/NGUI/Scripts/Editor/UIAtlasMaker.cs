...
public class UIAtlasMaker : EditorWindow
{
	...
	static public void AddOrUpdate (UIAtlas atlas, Texture2D tex,  Vector2 sizes = default(Vector2))
	{
		if (atlas != null && tex != null)
		{
			List<Texture> textures = new List<Texture>();
			tex.MakeReadable (true);
			tex.Resize(sizes.x == 0 ? tex.width : (int)sizes.x , sizes.y == 0 ? tex.height : (int)sizes.y);	
			textures.Add(tex);
