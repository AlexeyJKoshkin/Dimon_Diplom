		...
		if (mAtlas.spriteMaterial != null)
		{
			...
			if (sprite != null)
			{
				...
				if (tex != null)
				{
					...
					GUILayout.BeginHorizontal();
					New = (Texture2D)EditorGUILayout.ObjectField(New, typeof(Texture2D), allowSceneObjects: false);
					GUILayout.EndHorizontal();
					GUILayout.BeginHorizontal();
					sizes = EditorGUILayout.Vector2Field ("Size [X]/[Y]", sizes, GUILayout.Height(45));

//					if (GUILayout.Button ("Resize Current", GUILayout.Height (30f))) 
//					{
//						D
//						//sprite
//					}
					GUILayout.EndHorizontal();
					GUILayout.BeginHorizontal();
					if (GUILayout.Button("New", GUILayout.Height(30f)))
					{
						UIAtlasMaker.AddOrUpdate(mAtlas, New);
